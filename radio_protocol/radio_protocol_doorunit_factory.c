...
#include "radio_protocol_doorunit_factory.h"

#define DBG_TAG "RADIO_PROTOCOL_FACTORY"
#define DBG_LVL DBG_LOG
#include <rtdbg.h>

static void radio_frame_doorunit_factory_parse_heart(rx_format *rx_frame)
{
    ...
    tx_frame.tx_len = 3;
    radio_doorunit_factory_command_send(&tx_frame);
    LOG_I("radio_doorunit_factory_send\r\n");
}

void radio_frame_doorunit_factory_parse(rx_format *rx_frame)
{
    if(rx_frame->dest_addr != 10000001)
    {
        return;
    }
    ...
    {
    default:
        radio_frame_doorunit_factory_parse_heart(rx_frame);
        break;
    }
}

void radio_doorunit_factory_command_send(tx_format *tx_frame)
{
